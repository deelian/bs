...
function pngtm(thisimg){
	...
	thisimg.src="images/page/tm.gif";
	thisimg.width=i_w;
	thisimg.height=i_h;
}
...
$(function($){
	...
	$('.close').click(function(){
		$('#showdiv').hide();
		$('#videowen,#CuPlayer').text('');
		})
	$('.video_list a').click(function(){		
		$('#showdiv').show();
		cuplayerPlay($(this).attr("href"),$(this).attr("name"));
		$('#videowen').text($(this).attr("title"))
		var wentit=$(this).attr("title");	
		jiathis_config = {title:wentit}
		return false;
		})
	$('#stmenu a').hover(function(){
		$(this).addClass('current');
		},function(){
			$(this).removeClass('current');
			})
	
	$("body").append("<div class='backToTop' title='backhome'><span></span></div>")
	var $backToTopTxt = "backhome", $backToTopEle = $('.backToTop')
	...
	$('.backToTop').css({
		//right:Math.max(($('#warp').offset().left)-44,0)
		})
